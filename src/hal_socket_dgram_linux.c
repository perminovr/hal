...
bool UdpDgramSocket_joinGroup(DgramSocket self, const char *ip, const char *interface)
{
	if (self == NULL || ip == NULL || interface == NULL) return false;
	struct ip_mreqn imreqn;
	struct ifreq ifr;
	imreqn.imr_multiaddr.s_addr = inet_addr(ip);
	imreqn.imr_address.s_addr = htonl(INADDR_ANY);
	imreqn.imr_ifindex = if_nametoindex(interface);
...
DgramSocket EtherDgramSocket_create(const char *interface, uint16_t ethTypeFilter)
{
	...
	if (fd >= 0) {
		...
		idx = (int)if_nametoindex(interface);
		...
		if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, interface, IFNAMSIZ-1) < 0) goto exit_error;
...
{
	if (interface == NULL || addr == NULL) return false;
